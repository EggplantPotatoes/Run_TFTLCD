...
//			spi_flash_write_picture((uint16_t*)_acceshitu_1,PICTURE_6_ADDR);
//		    spi_flash_write_picture((uint16_t*)_acceshitupian6,PICTURE_5_ADDR);
		...
		if(key==KEY1_PRES)
		{
			...
			{
				spi_flash_read_picture(picture_rd,PICTURE_6_ADDR);
				HAL_DMA2D_Start_IT(&hdma2d,(uint32_t)picture_rd,(uint32_t)0xc0000000,1024,600);
				key_cnt = 0;
			}
//			for(i=0;i<100;i++)
